...
- Concierge routing and writing may use a model. Empty concierge model settings disable its router and writer. The router uses Pydantic JSON-schema output with one validation-feedback retry, then a logged heuristic fallback; the writer retries an empty response once before deterministic rendering.
- Model clients use `OpenAIChatCompletionClient` for OpenAI-compatible `/chat/completions` endpoints. `agent_runtime.run_structured` requests `response_format` and validates plans. Scooper's model selects from menu, availability, and quote operations; application code executes the validated plan. Invalid plans never execute tools, and endpoint failures surface explicitly.
- Specials are represented by `RoutingPlan.operations_intent`, not a keyword bypass before the model. Surprise reuses the live inventory-based Classic specials recipe, then quotes and verifies fulfillment. Keep fallback routing available for no-model operation and isolated capacity demos.
